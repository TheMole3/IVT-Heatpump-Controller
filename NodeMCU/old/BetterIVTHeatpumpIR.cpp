...
BetterIVTHeatpumpIR::BetterIVTHeatpumpIR() : HeatpumpIR()
{
...
void BetterIVTHeatpumpIR::send(IRSender& IR, uint8_t powerModeCmd, uint8_t operatingModeCmd, uint8_t fanSpeedCmd, uint8_t temperatureCmd, bool redo)
{
  ...
  // We have to redo if we're turning full power off.
  uint8_t redoPowerMode; // What mode should we use on the redo
  powerMode = SHARP_AIRCON2_FULL_EFFECT_OFF; // Default full effect off
  ...
  if(redo) 
  {
    switch (powerModeCmd)
    {
      case POWER_OFF:
        ...
        break;
      case POWER_ON:
        powerMode = SHARP_AIRCON2_MODE_ON;
        break;
      case POWER_FULL_ON:
        powerMode = SHARP_AIRCON2_FULL_EFFECT_ON;
        break;
    }
  }
  ...
  Serial.print("Fan speed: "); Serial.print(fanSpeed); Serial.print(" "); Serial.println(fanSpeedCmd);
  Serial.print("Temperature: "); Serial.print(temperature); Serial.print(" "); Serial.println(temperatureCmd);

  if (powerMode == SHARP_AIRCON2_FULL_EFFECT_ON) { // {0xAA, 0x5A, 0xCF, 0x10, 0x09, 0x61, 0x22, 0x00, 0x08, 0x80, 0x01, 0xF0, 0xC1} to set high power
    temperature = 22;
    fanSpeed = SHARP_AIRCON1_FAN_AUTO;
    operatingMode = SHARP_AIRCON1_MODE_HEAT;
  }
  ...
  sendSharp(IR, powerMode, operatingMode, fanSpeed, temperature);
   // Wait before redoing with the power mode off
  if(!redo) { // If we haven't redone yet, redo
    delay(1000);
    send(IR, powerModeCmd, operatingModeCmd, fanSpeedCmd, temperatureCmd, true); 
  }
}

void BetterIVTHeatpumpIR::sendSharp(IRSender& IR, uint8_t powerMode, uint8_t operatingMode, uint8_t fanSpeed, uint8_t temperature)
{
  ...
  // TODO Set the temperature for the weird modes
  // -2 = 0xA0
  // -1 = 0x90
  // 0 = 0x00
  // 1 = 0x10
